...
public class Rock : MonoBehaviour, IInteractable, IPointerEnterHandler, IPointerExitHandler
{
    ...
    {
        if (!player.IsHoldingTool("pickaxe")) return;
    ...
    {
        if (PlayerScript.Instance.IsHoldingTool("pickaxe"))
            TextCursorTooltipSingleton.Instance.SetTooltip("Mine rocks", null);
        else
            TextCursorTooltipSingleton.Instance.SetTooltip("Examine rocks", null);
    }
}
